  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center mb-5">
        ...
        <p className="text-xs text-gray-400 mt-1">Sign in to access your dashboard</p>
      </div>

      {/* Role tabs */}
      <div className="flex bg-gray-50/80 rounded-xl p-1 mb-3 gap-1">
        {Object.entries(ROLE_TAG).map(([role, tag]) => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1 py-2 px-1.5 rounded-[10px] text-xs font-medium transition-all duration-200 ${
                isActive
                  ? "bg-white shadow-[0_1px_4px_rgba(0,0,0,0.08)] border border-gray-100"
                  : "text-gray-400 hover:text-gray-600 hover:bg-white/60"
              }`}
            >
              <TagIcon className={`w-3.5 h-3.5 ${isActive ? info.color : ""}`} />
              {info.label}
            </button>
          );
        })}
      ...
      <button
        ...
        disabled={isBusy}
        className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-xs font-medium text-gray-700
